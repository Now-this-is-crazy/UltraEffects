...
public class HotbarHudRenderState {
    ...
    public void renderBackground(GuiGraphics context, Font textRenderer, int x, int y, float tickProgress) {
        int backgroundColor = adjustColor(ARGB.color(
                (int) (UEConfig.hotbarHudBackgroundOpacity * 255),
                UEConfig.hotbarHudBackgroundColorRed,
                UEConfig.hotbarHudBackgroundColorGreen,
                UEConfig.hotbarHudBackgroundColorBlue
        ));

        // Stats & Held Item
        UEUtil.renderRoundBox(context, x, y - 4, x + 127, y - 26, backgroundColor);

        if (this.experienceBar.isPresent()) {
            UEUtil.renderRoundBox(context, x, y - 96, x + 127, y - 109, backgroundColor);
        }

        // Held Item
        UEUtil.renderRoundBox(context, x, y - 27, x + 127, y - 95, backgroundColor);

        // Hotbar
        UEUtil.renderRoundBox(context, x + 128, y - 27, x + 146, y - 95, backgroundColor);
        ...
        // Offhand
        UEUtil.renderRoundBox(context, x + 128, y - 4, x + 146, y - 26, backgroundColor);
    }
    ...
    public void renderBars(GuiGraphics context, Font textRenderer, int x, int y, float tickProgress) {
        ...
        if (UEConfig.hotbarHudHealthNumberDisplay != UEConfig.HotbarHudHealthNumberDisplay.NEVER) {
            MutableComponent healthText = Component.literal("+" + UEConfig.hotbarHudHealthNumberDisplay.healthGetter.apply(this.healthBar, this.absorptionBar, tickProgress));
    ...
    public void renderItems(GuiGraphics context, Font textRenderer, int x, int y, float tickProgress) {
        if (this.getVisibility() < 1) {
            UEUtil.setAlphaOverride(context, this.getVisibility());
        }
        if (this.playerInventory != null) {
            ...
            if (!mainHandStack.isEmpty()) {
                ...
                int displayAlpha = switch (UEConfig.hotbarHudItemNameDisplay) {
                    case ALWAYS -> 255;
                    case NEVER -> 0;
                    case null, default ->
        ...
        UEUtil.resetAlphaOverride(context);
    }
    ...
    public float getVisibility() {
        if (this.chatFocused) {
            return switch (UEConfig.hotbarHudChatFocusModification) {
    ...
    public boolean shouldRender() {
        return UEConfig.hotbarHudEnabled && this.getVisibility() > 0 && !UEUtil.isRunningDeathScreenOverhaul();
    }

    public boolean shouldShiftChat(Minecraft client) {
        return this.shouldRender() && !UEUtil.isSpectator() && client.options.mainHand().get() == HumanoidArm.RIGHT;
    }
